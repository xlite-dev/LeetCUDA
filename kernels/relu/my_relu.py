...
_BUILD_DIR = os.path.join(_HERE, "build", "relu_lib")
os.makedirs(_BUILD_DIR, exist_ok=True)

lib = load(
    name="relu_lib",
    sources=[os.path.join(_HERE, "my_relu.cu")],
    extra_cuda_cflags=[
        "-O3",
...
def check_correctness(
    ...
    """Verify that perf_func(x[, out]) matches torch.relu(x)."""
    ref = torch.relu(x)
    if out is not None:
        out.fill_(0)
        perf_func(x, out)
...
def run_benchmark_for_all_test(check: bool = True):
    ...
    for S, K in SKs:
        ...
        if check:
            check_correctness(lib.relu_f32, x, "f32", y)
            check_correctness(lib.relu_f32x4, x, "f32x4", y)
            check_correctness(torch.relu, x, "f32_th")
        ...
        if check:
            check_correctness(lib.relu_f16, x_f16, "f16", y_f16, atol=1e-3, rtol=1e-3)
            check_correctness(lib.relu_f16x2, x_f16, "f16x2", y_f16, atol=1e-3, rtol=1e-3)
            check_correctness(lib.relu_f16x8, x_f16, "f16x8", y_f16, atol=1e-3, rtol=1e-3)
            check_correctness(lib.relu_f16x8_pack, x_f16, "f16x8pack", y_f16, atol=1e-3, rtol=1e-3)
            check_correctness(torch.relu, x_f16, "f16_th", atol=1e-3, rtol=1e-3)
...
def run_profiling_for_test(
    ...
    if dtype == torch.float32:
        if kernel_name == "relu_f32":
            run_profiling(lib.relu_f32, x, "profiling", y)
        elif kernel_name == "relu_f32x4":
            run_profiling(lib.relu_f32x4, x, "profiling", y)
        elif kernel_name == "relu_th":
            run_profiling(torch.relu, x, "profiling")
        else:
            raise ValueError(f"Unsupported kernel name: {kernel_name}")
    elif dtype == torch.float16:
        if kernel_name == "relu_f16":
            run_profiling(lib.relu_f16, x_half, "profiling", y_half)
        elif kernel_name == "relu_f16x2":
            run_profiling(lib.relu_f16x2, x_half, "profiling", y_half)
        elif kernel_name == "relu_f16x8":
            run_profiling(lib.relu_f16x8, x_half, "profiling", y_half)
        elif kernel_name == "relu_f16x8_pack":
            run_profiling(lib.relu_f16x8_pack, x_half, "profiling", y_half)
        elif kernel_name == "relu_th":
            run_profiling(torch.relu, x_half, "profiling")
        else:
            raise ValueError(f"Unsupported kernel name: {kernel_name}")
