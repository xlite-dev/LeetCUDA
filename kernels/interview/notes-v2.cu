...
static bool g_bench_only_cute = false;
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "--bench") == 0) {
      ...
      g_bench_fa = true;
    } else if (strcmp(argv[i], "--only-cute") == 0) {
      g_bench_mode = true;
      g_bench_only_cute = true;
  ...
  if (g_bench_mode) {
    ...
    if (!g_bench_only_fa && !g_bench_only_cute) {
