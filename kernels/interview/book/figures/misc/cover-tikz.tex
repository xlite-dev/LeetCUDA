...
\node[rotate=13, anchor=center, text=coverMuted!35, font=\ttfamily\footnotesize] at (2.4,24.3) {(4,8):(8,1)};
\node[rotate=-8, anchor=center, text=coverMuted!30, font=\ttfamily\footnotesize] at (18.6,21.4) {(128,128):(1,128)};
\node[rotate=6, anchor=center, text=coverMuted!30, font=\ttfamily\footnotesize] at (2.6,7.0) {(16,8):(8,1)};
\node[rotate=-13, anchor=center, text=coverMuted!30, font=\ttfamily\footnotesize] at (19.3,27.6) {((2,2),(2,2))};
\node[rotate=11, anchor=center, text=coverMuted!28, font=\ttfamily\footnotesize] at (19.2,17.2) {(3):(1)};
\node[rotate=-6, anchor=center, text=coverMuted!28, font=\ttfamily\footnotesize] at (2.2,2.4) {(8,16):(1,8)};
...
\begin{scope}[shift={(3.3,15.5)}, x={(0cm,-0.9cm)}, y={(0.9cm,0cm)},
...
\node[fill=colorx8!64] at (7,15) {127};
  \draw[white, line width=0.7pt, shift={(-0.5,-0.5)}] (0,0) grid (8,16);
  \draw[coverInk!35, line width=0.7pt] (-0.5,-0.5) rectangle (7.5,15.5);
\end{scope}

% 高亮 tile B = (2,2):(1,8) (右上角 2x2 tile, accent 粗框; scope 内 i0 行 0-1, i1 列 14-15)
\draw[accentBlue, line width=1.5pt, rounded corners=1.5pt,
  shift={(3.3,15.5)}, x={(0cm,-0.9cm)}, y={(0.9cm,0cm)}]
  (-0.5,13.5) rectangle (1.5,15.5);
\node[anchor=south, text=accentBlue, font=\ttfamily\bfseries\footnotesize]
  at (16.35,16.85) {$B=(2,2):(1,8)$};
\draw[-{Stealth[length=1.8mm]}, accentBlue, line width=0.8pt] (16.35,16.7) -- (16.35,16.0);

% layout 方程 (网格正下方, 居中)
\node[text=coverInk, font=\fontsize{13}{16}\selectfont] at (10.5,7.55)
  {$L = (8,16):(1,8)$};
% mode 标注: mode0 (stride 1) 向下, mode1 (stride 8) 向右
\node[rotate=90, anchor=center, text=coverMuted, font=\ttfamily\footnotesize] at (2.5,11.8) {mode\,0\,:\,(1)\;$\downarrow$};
\node[text=coverMuted, font=\ttfamily\footnotesize] at (10.5,16.35) {$\rightarrow$\;mode\,1\,:\,(8)};
\node[text=coverMuted, font=\ttfamily\footnotesize] at (10.5,6.8) {hue\,=\,2-col tile $\cdot$ ramp\,=\,row};

% ===== 底部声明 (细线框) =====
\node[draw=accentBlue!45, rounded corners=2pt, inner xsep=10pt, inner ysep=8pt,
  text=coverInk, text width=15.4cm, align=center, font=\small] at (10.5,4.6)
...
\node[text=coverMuted, font=\small] at (10.5,2.85)
  {LeetCUDA Project $\cdot$ 2026 年 9 月};
\end{tikzpicture}
\end{document}
