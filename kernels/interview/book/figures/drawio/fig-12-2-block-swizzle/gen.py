...
for r in range(LR):
    for c in range(LC):
        f = "#fde9d0" if r < 2 else GRAY
        s = ORANGE if r < 2 else "#d9e2ec"
        box(LGX + c * CW, LGY + r * CH, CW - 2, CH - 2, fill=f, stroke=s)
text(LGX, LGY + LR * CH + 8, 300, 18, "x: 0 ............ 127 (T_n=128 列)", fs=12, fc="#627d98")
...
text(RGX, LGY + (RR + 4) * CH + 8, 400, 20, "resident = 16 x 14 近方块（同 220 tiles）", fs=13, fc="#2e8540", bold=True)
...
text(56, FY + 36, 310, 24, "= 16640 * K * 2B", fs=14, fc="#8c2f0f", bold=True)
box(400, FY, 330, 70, "", fill="#c9e4c8", stroke="#2e8540")
text(416, FY + 8, 310, 24, "swizzle：(14*128 + 16*128) * K * 2B", fs=12.5, fc="#1e5631")
text(416, FY + 36, 310, 24, "= 3840 * K * 2B", fs=14, fc="#1e5631", bold=True)
box(760, FY, 330, 70, "footprint 缩小 4.3x", fill="#e7ecf3", stroke=BLUE, fs=16, bold=True, fc=BLUE)
text(40, FY + 78, 900, 20, "实测 L2 sector hit rate：64.45% -> 96.15%（PRO 5000，2048x16384x4096 HGEMM）", fs=13, fc="#48586a")
