...
def fig_20_4():
  ...
  text(230, 572, 560, 20, "*(base_ptr + offset) —— 线性索引 5 命中行主序第 5 号元素 'f'", fs=14, fc=PURPLE)
...
def fig_20_5():
  ...
  text(30, 42, 1060, 20, "C(p, q) = A(B(p, q))：先在内层 B 上查 offset，再按行主序换算成 A 的坐标，最后在 A 上取值回填", fs=12.5, fc="#48586a")
  CW, CH = 46, 30
  # 左上：C 提问态（8x2 空，(4,1)=?）
  LX, LY = 70, 100
...
def fig_20_7():
  ...
  text(410, 118, 200, 18, "shape: (4)", fs=11.5, fc=ORANGE)
  text(410, 136, 200, 18, "stride: (1)", fs=11.5, fc=ORANGE)
  ...
  text(410, 378, 200, 18, "shape: (4)", fs=11.5, fc=BLUE)
  text(410, 396, 200, 18, "stride: (1)", fs=11.5, fc=BLUE)
  # (b) stride：stride (4, 1)
  ...
  arrow([(730, 260), (790, 260), (790, 430), (850, 430)], color=BLUE, width=1.8, dashed=1)
  text(700, 408, 200, 18, "right_inverse(layoutA)", fs=12, fc=BLUE)
  grid(860, 372, 48, 30, 4, 4, vals=list(range(16)), stroke=BLUE, fs=10.5)
  text(860, 502, 210, 18, "right_inv = layoutA 自身", fs=12, fc=BLUE, bold=True)
  text(860, 520, 210, 18, "shape: (4, 4), stride: (4, 1)", fs=11.5, fc=BLUE)
  text(60, 548, 1080, 20, "CuTe v4.6.1 实测：stride 0 广播布局的两逆相同——去重投影 (4):(1)；双射且对合的布局，两逆均为其自身", fs=12.5, fc="#48586a")
