...
5. **drawio 管线**（知乎图→正式图主路径）：`drawio-reconstruction` skill 为主（inventory→重建→审查闭环），`drawio-diagram-builder`/`drawio-flow-forge` 辅助（ASCII 升级/新建）。**执行模式：主 agent 直接做，不派 task agent**（闭环子代理实测易卡死，用户 2026-09-11 拍板）。审查结论如实记 audit（自审需标注）。**质量三查（每图必过）**：XML well-formed + 文字覆盖（COVER=0）+ 渲染宽不越界（见「drawio 制图铁律」）。**`.drawio` 源文件必须入库**（用户要求可手改，勿只提交 PNG）。例外：ch26b 六图为用户指定的 TikZ inline（直接写在章节 tex 内），新图默认仍走 drawio 主路径。
