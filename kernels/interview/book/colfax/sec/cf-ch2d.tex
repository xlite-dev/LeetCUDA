...
\begin{definition}\label{definitionofnestedtupleattributes}
    ...
    \begin{cases}
        0 & P = *\\
        1 + \displaystyle\max_{1 \leq i \leq r} (\depth(P_i)) & P = (P_1,\dots,P_r) \text{ 是一个由轮廓组成的元组。}
    \end{cases}
    \]
    \end{itemize}
\end{definition}
...
\[
\begin{aligned}
X &= 27 & \quad \rank(X) &= 1, \quad \len(X)  = 1, & \depth(X) &= 0, \quad \size(X) = 27 \\
X &= (2,10,5) & \quad \rank(X) &= 3, \quad \len(X)  = 3, &\depth(X) &= 1, \quad \size(X) = 100 \\
X &= (((3,4),2,2),8,9), & \quad \rank(X) &= 3, \quad \len(X)  = 6, & \depth(X) &= 3, \quad \size(X) = 3096 \\
X &= (((),()),(2,(5,5))), & \quad \rank(X) &= 2, \quad \len(X)  = 3, & \depth(X) &= 3, \quad \size(X) = 50 \\
\end{aligned}
\]
\end{example}
