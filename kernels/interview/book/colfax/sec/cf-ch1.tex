...
\begin{enumerate}
    \item 把操作数矩阵 $A$ 与 $B$ 划分为若干块（tile），
    \item 计算各块之间的矩阵乘积，以及
    \item 合并这些部分结果以得到完整结果 $AB$。
\end{enumerate}
譬如，我们可以把 $4 \times 8$ 矩阵 $A$ 划分为 $2 \times 2$ 的块，如下所示。
\[
...
\noindent 现在假设我们想切出 $A$ 的各个块，并设 $A$ 在内存中按列主序格式存放。为此，可以按如下方式手工计算偏移：对第 $(i,j)$ 个块，索引其左上角元素的偏移由 $2i + 8j$ 给出。另一方面，为了更好地组织这一计算，我们可以使用块的{\bf 交错}（interleaved）布局
\\
...
\noindent 其中列由 $A$ 的块给出，行由块形状内的坐标给出。这里我们用{\bf 余字典序}（colexicographic ordering）来线性地枚举块以及块内的坐标，因此布局 $L^\tiled$ 的顶层形状为 $(4, 8)$。

然而，请注意，$L^\tiled$ 所呈现的交错模式意味着，对任何步长 $a,b$，它都不能表示为布局 $(4,8):(a,b)$。作为替代，我们可以把形状 $(4,8)$ 的各模式分解因式，并定义
\[ L^\tiled = ((2,2),(2,4)):((1,4),(2,8)). \]

\noindent 这样，前面的偏移计算 $2i + 8j$ 就体现为在坐标 $(0, (i,j))$ 上对 $L^\tiled$ 求值，而块布局本身则由第一个模式给出。于是，在为 $A$ 赋予布局 $L^\tiled$ 而得到 $A^\tiled$ 之后，我们就可以把 $A$ 的第 $(i,j)$ 个块取为切片
...
% 40 行记号表整体高于版心，必须允许行间分页，否则产生牺牲性空白页
{\allowdisplaybreaks
\begin{align*}
