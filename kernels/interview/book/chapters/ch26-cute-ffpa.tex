...
TiledMma 定下来之后，每个线程持有多少寄存器就定了。这里有两个恒等式，后面要讲的「无缝交接」就建立在它们上。
...
\begin{tikzpicture}[
  ...
  \draw[very thick,tkGreen,fill=tkFillGreen] (12.35,-2.95) rectangle (13.6,-2.25);
...
还有一个容易忽略的点：B fragment 的布局对 4 个 M-warp 是\emph{重复}的
...
\textbf{CuTe 在 Split-D 上的收益与代价都不在运行时，而在「类型
