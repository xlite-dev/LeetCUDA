...
16 个元素。这就是 SA3 所谓 C2 挑战（SF 格式自身的精度损失）：若块
$\mathrm{amax}$ 落在 ue4m3 的窄域之外，或行内动态范围极端（causal 早
行 $P$ 近 one-hot），SF 编码误差就会放大。两级 P 量化（下一章）正是
为了把块的 $\mathrm{amax}$ 铺满 ue4m3 的域。
...
\begin{equation}
  ...
  \Delta S[b,h,m,n]=q_m[b,h,m,:]\cdot\bigl(K[b,h,n,:]-\bar k\bigr)
  \label{eq:31-deltas}
\end{equation}
...
式（\texttt{--fp4-smooth-v}，三族 kernel 全支持）。
...
（$\mathrm{mean}(Hq)=H\,\mathrm{mean}(q)$，$HH^\top=I$）⇒ 均值与旋转
...
每线程持 64 元素 slice（恰一个 SF 的 4 列组），$kHeadDim/64$ 个
16 元素向量迭代。四个模板 bool 各管一件事：
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/fig-31-1-nvfp4-block.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/fig-31-2-fp4-pipeline.png}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.98\textwidth]{figures/fig-31-3-kv-perm32.png}
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}rlllll@{}}
    ...
    128 & 2.52 / 5.11 & 437T & 1.67 / 2.68 & 330T & 1.61--2.04x \\
    192 & 3.48 / 7.14 & 474T & 2.40 / 3.81 & 344T & 1.59--2.05x \\
    256 & 4.97 / 10.46 & 443T & 3.54 / 5.27 & 311T & 1.47--2.11x \\
    \bottomrule
  \end{tabular}
\end{table}
...
（dense 0.15 / causal 0.70），不要用 fp8 的 0.05 级容径去卡 fp4。
...
\begin{enumerate}[itemsep=2pt,topsep=2pt]
  ...
  \item \textbf{e2m1 需要显式饱和}：手写 round(x/s) 不带 clamp，越界
        值在 \texttt{cvt} 的非 satfinite 变体里会产 NaN；统一走
        \texttt{satfinite} 变体（L41/L64）。
...
\begin{enumerate}[itemsep=2pt,topsep=2pt]
  ...
  \item smooth-V 精确性：softmax 权重和为 1 ⇒ 减均值是恒等变换，
        只收动态范围。
  ...
  \item fused WHT 的蝶形拆分：thread-local 对原地做，只剩两级
        \texttt{shfl\_xor}；均值/旋转可交换 ⇒ $\Delta S$ 留在未旋转
        域。
\end{enumerate}
