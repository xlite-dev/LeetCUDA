...
NCU 的 stall 剖像一锤定音：\texttt{math\_\allowbreak{}pipe\_throttle} 1.32
...
nsys 与 ncu 不是大小写不同的同一个东西，它们的采集原理完全不同：
...
\texttt{cuda\_gpu\_kern\_sum} 按 kernel 聚合总时长与占比，直接给出「该优化
谁」的答案；\texttt{cuda\_gpu\_trace} 逐 launch 列时间戳，用来看相邻 kernel
间的空隙与多流 overlap。首次对旧 report 运行时报
\texttt{Existing SQLite export found} 时，加 \texttt{-{}-force-export=true}
强制重新导出。

\subsection{体积控制：一次 500GB 事故}

nsys 的中间数据默认写到 \texttt{/tmp/nvidia/nsight\_systems}，体积 $\approx$
事件数 $\times$ 时长。对大模型推理（多模型链、大 seqlen、多次 repeat）无脑
...
\item \textbf{优先 torch.profiler}：只想看 kernel 时间表时，
  \texttt{torch.profiler} 输出 chrome trace 落在 \texttt{.tmp/}，零外部临时
...
\texttt{-{}-capture-range=cudaProfilerApi} 配合代码里的开关，把采集窗口钉在
目标区间：
...
\texttt{persist\_d\_ws\_fwd\_cute\_fp8\_sm120}，实际实例化名比这更长）。
...
\texttt{RmProfiling\allowbreak{}AdminOnly} 标志为 1，
此时 \texttt{-{}-set full} 直接报 \texttt{ERR\_NVGPUCTRPERM}，且容器内无法
...
\texttt{sm\_\_throughput}（SM\%）与 \texttt{gpu\_\_compute\_memory\_throughput}
...
（\texttt{sm\_\_warps\_active}，实际运行均值，受 tail effect 与负载不均
拖累）。NCU 的 Occupancy section 会明确指出 \texttt{launch\_\_occupancy\_limit\_registers} /
\texttt{\_\_shared\_mem} / \texttt{\_\_blocks} 哪个是当前限制源。一个
fp8 attention kernel 的实例：REG:255 直接把 \texttt{Block Limit Registers} 压到 1——
1 CTA/SM、8 warps，occupancy 16.67\%，此时想提并行度只有降寄存器或换结构
...
\texttt{launch\_\_registers\_per\_thread} 给出每线程寄存器数，硬上限
\textbf{255}（每线程）。超限时编译器把变量溢出到 local memory（SASS 里表现为
\texttt{LDL}/\texttt{STL} 指令，NCU 里看
\texttt{l1tex\_\_t\_bytes\_pipe\_lsu\_mem\_\allowbreak{}local\_op\_\_ld/st.sum} 是否非零）。
编译期排查用 \texttt{-{}-ptxas-options=-v} 与
...
\item \textbf{寄存器预算可以算出来}。attention 的 O 累加器每线程占
  ...
  行——一个 warp 摊到的 O 分量总数）；$k_{Br}{=}128$ 时为
...
\texttt{smsp\_\allowbreak{}average\_warps\_\allowbreak{}issue\_stalled\_\allowbreak{}*\_per\_issue\_active.ratio}
...
\texttt{long\_scoreboard} & 等 gmem/TMA 数据 & 装载不足：\texttt{cp.async}/TMA、加深 stage、prefetch \\
\texttt{short\_scoreboard} & 等 smem（MIO 队列） & LDS 链太长：swizzle 布局、减少 smem 往返 \\
\texttt{math\_pipe\_throttle} & 数学管线排队满 & \textbf{好信号}：计算饱和。保持或进一步减 FLOPs \\
\texttt{sleeping} & warp 休眠（mbarrier 退避） & \textbf{纯浪费}：producer/consumer 失衡或空转自旋，查 WS 结构 \\
\texttt{barrier} & 等显式栅栏 & 同步过频：减少 \texttt{\_\_syncthreads} 点、缩 block \\
\texttt{not\_selected} & 就绪但调度器选了别人 & \textbf{健康}：有冗余并行度可隐藏延迟 \\
\texttt{selected} & 正在发射（非停滞） & 基线，越大越好 \\
\bottomrule
\end{tabular}
\end{table}

读 stall 剖像的关键是\textbf{分好坏}：\texttt{math\_pipe\_throttle} 与
\texttt{not\_selected} 高说明机器在干活；\texttt{sleeping} 高几乎总是结构
问题；\texttt{wait} 与 \texttt{long\_scoreboard} 的\textbf{相对大小}直接
...
\begin{itemize}
\item \textbf{L1/TEX}：\texttt{l1tex\_\_t\_sector\_hit\_rate.pct}。低于
  20\% 且数据本该复用时，考虑 tiling 进 smem 或改访问模式。注意 smem 与 L1
  \textbf{共池}（GB202 实测每多占 16KB smem，kernel 约慢 4--5$\mu$s），
  「多分 smem」不是免费的。
\item \textbf{L2}：\texttt{lts\_\_t\_sector\_hit\_rate.pct}。大 GEMM 上低
  命中可用 CTA swizzle / StreamK 提数据局部性；推理场景 K/V 重复读高命中
  是常态。
\item \textbf{DRAM}：\texttt{dram\_\_throughput}（峰值百分比）。贴满
  （$>$70\%）说明带宽墙已到，优化方向只剩减流量（混合精度、融合、去
  物化）。一条量化前处理链实测贴到设备峰值带宽 1.04TB/s（理论
  1344GB/s）——此时「融合省 launch」已无杠杆，只有减流量才有，这个
  判断直接由 DRAM 指标给出。
\item \textbf{load/store 与合并访问}：global load 的每请求扇区数
  （\texttt{l1tex} 前缀的
  \texttt{average\_t\_sectors\_\_per\_request\_\allowbreak{}...} 系列指标）
...
\texttt{l1tex\_\_data\_bank\_conflicts\_pipe\_lsu\_mem\_\_shared\_op\_ld/st.sum}
...
判据正是「\texttt{math\_pipe\_throttle} 降了、tensor pipe 升了」——两条
...
\item \textbf{latency-bound 分流}（看 stall 前二名）：\texttt{wait} 或
  \texttt{math\_pipe\_throttle} 主导 $\to$ 依赖链/发射受限，对策是拆链
  （拆 CTA、拆 warp 分工）或增并行 warp；\texttt{long\_scoreboard} 主导
...
\item \textbf{查 occupancy 是否是限制源}：若 achieved 低且 limiter 是
  registers/smem，先解决资源（缩 tile、降 stage、\texttt{\_\_launch\_bounds\_}），
...
链每个 kernel 都已贴设备峰值带宽 1.04TB/s——融合只能省 launch 不能减流量，
...
\item \textbf{动态}：NCU 确认 \texttt{Block Limit Registers=1}（REG:255 的
  直接后果），occupancy 16.67\% 且 smem 守卫恒空转——限制源唯一。
...
\begin{itemize}
\item \texttt{math\_pipe\_throttle} \textbf{1.32}（第一名）、
  \texttt{wait} 0.82、\texttt{long\_scoreboard} \textbf{0.04}、tensor
  pipe 49\%、255 regs、1 CTA/SM。
\item 判读：\texttt{long\_scoreboard} 近零 $\Rightarrow$ 装载不是瓶颈，
...
3.5\% 涨到 22.3\%（SASS 采样里 \texttt{SYNCS.PHASECHK} mbarrier 自旋占
...
「没病」而是 latency-bound，答案在 stall 剖像里；stall 分好坏，
\texttt{math\_pipe\_throttle} 高是 Tensor Core 喂满的勋章，\texttt{sleeping}
高是结构问题的警报；寄存器 255 墙、spill 与 occupancy 是同一份预算的三种
花法，读数时要一起读。工具路径：ncu/nsys 随 CUDA Toolkit 安装（通常在
\texttt{/usr/local/cuda/bin}），不在 PATH 时用绝对路径调用。
