...
\CuTe 为一组布局提供了紧凑的表示，这组布局严格大于用传统扁平形状（flat shape）与扁平步幅（flat stride）所能表示的布局集合。相比之下，CUTLASS v2（Kerr 等，2017）与 Thunderkittens（Spector 等，2024）等库对每种布局进行单独的手工实现。这种方式劳动密集、易出错，且需要大量开发时间。为直观说明其复杂度：CUTLASS v2 代码库中包含近 300 种分别实现的布局，分布在 87 个文件中，总计约 55,000 行代码。此外，CUTLASS v2 中的许多算法被设计为仅能配合这些布局中一个有限的子集工作，加剧了维护与可扩展性方面的挑战。作为对比，\CuTe 的核心布局表示连同操作布局的相关代数仅需 3,000 行代码，便足以表示 CUTLASS v2 中的全部 300 种布局乃至更多。用 \CuTe 实现的算法可以对其输入的秩（rank）或形状施加约束，但与任何满足这些前置条件的布局保持相容。将算法逻辑与具体的数据布局或线程布局解耦，使代码更灵活、更可组合。

上述优势已获认可，\CuTe 如今构成了 CUTLASS v3、CUTLASS v4 与 CuTe DSL 的基础，它们全都构建在 \CuTe 的核心布局表示与代数之上。现代张量指令所需的复杂数据布局与划分（partitioning）模式，都通过 \CuTe 单一、一致且可组合的表示来表达与操作；这一表示在 NVIDIA 指令集体系结构的多个世代中始终保持稳健。

本节中，我们详细说明如何仅凭布局与张量表示，为两种最基础的算法——{\tt COPY}（拷贝）与 {\tt GEMM}（矩阵乘）——提供强大的通用实现。这些算法以 \CuTe 张量实现，用以说明逻辑实现能够适用于广泛的应用场景。这些算法尽管自身往往已是最优，仍可作为各类优化版本的出色通用参考实现。第~\ref{sec:layoutalgebra} 节的布局代数提供了检查与操作布局的方法，用以完成这些优化。
...
\noindent 其中，前置条件规定两个张量的大小（size）相等；这一条件等价地写在张量参数的注释中——两个张量均与形状 $N$ 相容。
...
当源张量与目标张量的 \texttt{idx2crd} 函数求值的计算开销很低时（例如张量形状为静态已知时），上述实现实际上从不会生成动态的坐标变换。在这类情形下，循环可以被展开，\texttt{idx2crd} 可以静态地作用于循环索引 \texttt{i}，而 \texttt{inner\_product} 计算在求取偏移时的开销微乎其微。这是静态分析与优化的一种形式，因为布局的形状和/或步幅往往在编译期便已可知、可被编译器利用。倘若 \texttt{idx2crd} 确实带来运行时开销，所给的这一实现仍可作为稳健的参考，用于验证那些可能进一步检查布局的定义域（domain）、并利用第~\ref{sec:layoutalgebra} 节详述的操作来变换布局的优化版本。
...
只要改变各张量的布局，这一简单的 \texttt{GEMM} 实现（以及一个 \texttt{batched-GEMM} 扩展）便能覆盖多种应用。表~\ref{tab:gemm} 展示了一些常见应用与示例布局，其中包括 BLAS GEMM 的全部 N-T 变体，以及 BLIS GEMM 的通用步幅变体（\texttt{dm*}、\texttt{dn*}、\texttt{dk*}）。它还可以充当完全通用的张量-张量收缩（{\tt GETT}）：其中各张量按逻辑上的行模式（mode）、列模式、归约模式与批量模式分组，被折叠成适当的矩阵形状（Shi 等，2016）。构造一个实现 {\tt im2col} 变换的布局（作为 \CuTe 布局的函数复合）（Chetlur 等，2014），还能让 \texttt{GEMM} 实现 {\tt CONV}——许多现代机器学习应用的核心。
...
通过对融合乘加（fused-multiply-add）操作加以抽象、并提供足够强大的分块（tiling）工具，这一算法可被适配并在体系结构层次结构的每一层递归地应用。
