...
上述优势已获认可，\CuTe 如今构成了 CUTLASS v3、CUTLASS v4 与 CuTe DSL 的基础，它们全都构建在 \CuTe 的核心布局表示与代数之上。现代张量指令所需的复杂数据布局与分块（partitioning）模式，都通过 \CuTe 单一、一致且可组合的表示来表达与操作；这一表示在 NVIDIA 指令集体系结构的多个世代中始终保持稳健。
...
通过对融合乘加（fused-multiply-add）操作加以抽象、并提供足够强大的铺瓦（tiling）工具，这一算法可被适配并在体系结构层次结构的每一层递归地应用。
