...
\end{align}
布局的补是一个在原布局的陪域（codomain）之中、但不在其像（image）之中生成元素的布局。在复合（Composition）中，布局 $\v{L}$ 常被用作对另一个布局的间接寻址，因此补 $\v{L}^*$ 就是指向 $\v{L}$ 所遗漏元素的那个布局。有了 {\tt complement}，就能经由\emph{逻辑切分}（logical divide）把布局切开（第~\ref{sec:logical_divide}~节），也能经由\emph{逻辑乘积}（logical product）把布局重复、扩展（第~\ref{sec:logical_product}~节）。

本节只关注陪域 $\D$ 为 {\tt HTuple}($\Z$) 的布局：即整数，或对某个轮廓 D 而言 $\D = \Z^D$ 的坐标。弱同余条件式~\eqref{eqn:comp:congruence} 因此成立。像互不相交条件（式~\eqref{eqn:comp:disjoint}）保证补所生成的值与原布局生成的值互不相同：原布局这边取的是有限定义域 $\Z(\v{L})$，补那边取的是无穷扩展定义域 $\Z^{\v{L}^*}$，因此无论补的形状被截断还是被扩展（例如通过与右侧的布局复合），它所生成的值始终与原布局不同。最后，像有序条件（式~\eqref{eqn:comp:ordered}）确立了补布局的唯一性。
...
\end{align*}
其中 $\v{A}^*$ 是 $\v{A}$ 的补。结果的第一个模（mode）恰好就是输入布局 $\v{A}$，而第二个模在元素形状（shape）与元素顺序上与输入布局 $\v{B}$ 相容。第一个模，即 $\v{A}$ 布局，通常被称为“块”，它在“网格”布局 $\v{B}$ 上被重复。%Because the purpose of the logical product is to expand the domain and codomain of $\v{A}$, this does not require that $(\v{A},\v{A}^*)$ is surjective onto $\Z_{\norm{\v{B}}}$.

考虑这样的例子：行主序（row-major）布局 $(\textcolor{red}{3},\textcolor{red}{4}):(\textcolor{red}{4},\textcolor{red}{1})$ 块，要在列主序（col-major）布局 $(\textcolor{blue}{2},\textcolor{blue}{5}):(\textcolor{blue}{1},\textcolor{blue}{2})$ 网格中复现：
\begin{align*}
...
\end{align*}
其中布局 $1:12$ 是 $(\textcolor{red}{3},\textcolor{red}{4}):(\textcolor{red}{4},\textcolor{red}{1})$ 的补。结果的第一个模中可以直接得到原始块，而第二个模以 $2\times 5$ 列主序的顺序把原始块铺 10 次，互不重叠。

再换一个例子，考虑布局 $(\textcolor{red}{4},\textcolor{red}{8}):(\textcolor{red}{20},\textcolor{red}{2})$ 块在行主序布局 $(\textcolor{blue}{3},\textcolor{blue}{2}):(\textcolor{blue}{2},\textcolor{blue}{1})$ 网格中复现：
\begin{align*}
...
\end{align*}
其中布局 $(2,1):(1,80)$ 是 $(\textcolor{red}{4},\textcolor{red}{8}):(\textcolor{red}{20},\textcolor{red}{2})$ 的补。此时网格在交错排布这些块，以产生结果的最小陪域。补布局告诉结果如何为布局 $\v{B}$ 设定步幅，而布局 $\v{B}$ 告诉结果如何为补布局排序并定形。

\paragraph{相关乘积}

逻辑乘积产生一个 2 阶布局，其中第一个模是原始块，第二个模被解释为该块的网格，即“分块”。

逻辑乘积对 $\v{A}$ 与 $\v{B}$ 的阶（rank）和相容性都没有限制；操作 {\tt blocked\_product} 则更直观，它利用输入的阶来构造结果的形状。直觉上，一个 $3 {\times} 4$ 布局与一个 $2 {\times} 5$ 布局相乘，就该得到一个 $6 {\times} 20$ 布局。{\tt blocked\_product} 要求 $\v{A}$ 与 $\v{B}$ 的阶相同，
\begin{align*}
\text{阶}(\v{A}) = \text{阶}(\v{B})
\end{align*}
...
\end{align*}
便得到一个 $6 {\times} 20$ 布局。图~\ref{fig:blockedproduct} 画出了这个布局，其中每个 $3 {\times} 4$ 块都施以底纹。“块状”之名由此而来：结果布局中的每个 $3 {\times} 4$ 块，都恰好是原始 $3 {\times} 4$ 块的平移版本。
...
\caption{行主序 $(3,4):(4,1)$ 块与列主序 $(2,5):(1,2)$ 分块的 {\tt blocked\_product}。}
\label{fig:blockedproduct}
\end{figure}

操作 {\tt raked\_product} 与块状乘积类似，只是各模以相反的顺序拉链（zip）组合：
\begin{align*}
\begin{array}{cccc}((\textcolor{red}{3}, & \textcolor{red}{4}), & (\textcolor{blue}{2}, & \textcolor{blue}{5})) \\ ((\textcolor{red}{4}, & \textcolor{red}{1}), & (\textcolor{blue}{12}, & \textcolor{blue}{24})) \end{array}
\ \Longrightarrow \
...
\end{align*}
其效果是 $3 {\times} 4$ 块被“耙”（raked）在 $2 {\times} 5$ 块之上，而非“块状”排布。图~\ref{fig:rakedproduct} 画出了得到的布局，其中每个 $3 {\times} 4$ 块都施以底纹。
...
\caption{行主序 $(3,4):(4,1)$ 块与列主序 $(2,5):(1,2)$ 分块的 {\tt raked\_product}。}
...
逻辑乘积还有不少其它版本，都会对模额外分组与重排，让结果更便于使用。
...
\end{align*}
其中 $\v{B}^*_{\abs{\v{A}}}$ 是 $\v{B}$ 相对于布局 $\v{A}$ 的尺寸所取的补。这个操作要保留 $\v{A}$ 的全部元素、只是把布局 $\v{A}$ 一分为二，所以我们要求 $\v{B}^\bigstar = (\v{B},\v{B}^*_{\abs{\v{A}}})$ 满射到 $\Z_{\abs{A}}$。这通常要扩展 $\v{B}^*$ 的尺寸，使 $\abs{\v{B}^\bigstar} \geq \abs{\v{A}}$。同时，我们还要求补 $\v{B}^*$ 在如下意义上“补全”布局 $\v{B}$：$\v{B}^\bigstar = (\v{B},\v{B}^*_{\abs{A}})$ 具有自反广义逆 $\v{B}^+$，满足
\begin{align}
\v{B}^\bigstar \v{B}^+ \v{B}^\bigstar = \v{B}^\bigstar \\
\v{B}^+ \v{B}^\bigstar \v{B}^+ = \v{B}^+
\end{align}

与逻辑乘积操作类似，结果布局 $\v{A} \circ \v{B}$ 的第一个模通常被称为“块”，它在“网格”（即“分块”）布局 $\v{A} \circ \v{B}^*_{\abs{\v{A}}}$ 上被重复。参数 $\v{B}$ 被称为“分块器”，它可以扩展到布局之外，第~\ref{sec:tiler}~节会讲到。

先看个直接的例子。常见需求是从另一个布局中取出某组逻辑元素，比如每隔三个取一个，用复合就能做到：
\begin{align*}
...
\end{align*}
其中 $3:1$ 是 $8:3$ 在尺寸 24 之下的补。结果的第一模按要求仍然包含左手边布局中每隔三个取一个的元素，而结果的第二模是这些“块”在原布局中的个数（本例中为 3）以及这些块之间的步幅。补用来确定缺失了哪些元素、以何种顺序保留它们。

\paragraph{相关切分}

与复合类似，逻辑切分也经常按模来做，沿用同样的记号与策略
\begin{align*}
\v{A} \oslash \langle \v{B},\v{C} \rangle = (\v{A}_0,\v{A}_1) \oslash \langle \v{B},\v{C} \rangle = (\v{A}_0 \oslash \v{B}, \v{A}_1 \oslash \v{C}).
\end{align*}
这么设计的道理是：列和行可以各自独立地执行这些操作，然后视需要重排这些模、构造新布局，与布局乘积类似。例如，回到第~\ref{sec:tiler}~节中的分块器，将这些分块器用于逻辑切分而非复合，就得到分块器所指向的 $4 {\times} 8$ 子布局组成的 $2 {\times} 2$ 布局。图~\ref{fig:tilers_divide} 画出了这一点：可以把每个单独施以底纹的数据块，想成有一个 $2 {\times} 2$ 布局映射到它上面。
...
\end{subfigure}
\caption{将 $8 {\times} 16$ 布局切分为 $4 {\times} 8$ 块、并以 $2 {\times} 2$ 分块的分块器示例。}
\label{fig:tilers_divide}
\end{figure}

例如，给定布局 $(\textcolor{red}{8},\textcolor{red}{16}):(\textcolor{red}{20},\textcolor{red}{1})$，我们想抽取一个“块”：沿每列向下取 4 个连续元素、沿每行每隔一个取一个元素。按模的逻辑切分给出
\begin{align*}
...
\end{align*}
其中“块”模标为蓝色，“其余”模标为黑色。结果布局仍保持 $8 {\times} 16$，而分块器所指向的元素已被置换到最前面的 $4 {\times} 8$ 块中。
...
\end{align*}
这生成了一个“块”模（蓝色），它恰好是分块器所指向的子布局；以及一个“其余”模（黑色），它是遍历各块的子布局。操作 {\tt zipped\_divide} 总是返回一个 2 阶布局结果，其第一模恰好是与分块器的复合，第二模是与补的复合。

软件中一个非常常见的模式是：把数据布局分块成一个块网格，然后为每个处理单元选出特定的块。在 Python 参考实现 PyCuTe（NVIDIA，2026）中，其形式如下：
\begin{python}
...
\end{python}
这一模式与第~\ref{sec:layouttv}~节的线程-值划分非常相似，只不过真正指定的只有块模，缺失的“网格”模由分块器的补计算出来。这个形状为 $(\text{块}, \text{网格})$ 的“补全”分块器随后与数据复合，产生块网格；再在第二模上按块标识符切片，即可为某个处理单元抽取特定的块。

逻辑切分同样可以构造出不少其它版本，做法相同：对模额外分组与重排，得到便于使用的结果。

\wph{结论}

本文提出了 \CuTe，一个用于表示与操纵张量布局的数学框架，以应对日益复杂的现代 GPU 架构。它的分层布局表示，为编写与管理当代硬件上专用张量指令所需的复杂数据布局与划分（partition）模式提供了通用方法；它的丰富代数运算，则为高性能线性代数 kernel 开发中布局的操纵与新布局的生成提供了一条有原理依据的途径。二者结合，该表示与代数能够支持复杂的布局与划分、关注点分离，以及静态分析与优化。

\CuTe 布局表示的表达能力已在若干关键应用中得到体现。\CuTe 便于将多种多样的算法，例如广义张量-张量收缩（GETT）与卷积（CONV），表示为通用矩阵乘法（GEMM）的实例，代码得以复用，算法也因此统一。例如，\CuTe 允许矩阵无论其具体数据布局如何都用 $(m,n)$ 坐标（coordinate）索引，让通用算法与具体布局保持正交；要求特定布局的算法或指令则能够静态地检查、验证并推演它们的张量参数。

\CuTe 布局代数支持对张量布局做静态分析与变换，用以实现并验证通用算法。例如，它可以为 \texttt{COPY} 操作推导最大向量化机会，验证矩阵乘累加（\texttt{MMA}）与 \texttt{COPY} 指令的输入输出布局，并推导出最优的共享内存布局，包括避免 bank 冲突的 swizzle 模式。有了这些能力，布局管理从易错的手工过程变成了一套系统、可验证的做法。为支持对本文定义的学习、实验与验证，纯 Python 参考实现 PyCuTe（NVIDIA，2026）已在 \url{https://github.com/NVlabs/CuTe} 公开发布。

\CuTe 的实际影响体现在它已成功部署于生产系统，最显著的就是 NVIDIA CUTLASS v3 与 v4 库都建在它之上。在 CUTLASS 以及 FlashAttention 等应用中的性能评估表明，\CuTe 的抽象不引入任何性能开销，软件开发却显著加快。采用这一框架的结果是：手工优化代码的性能特征得以保持，开发复杂度与开发时间大幅下降。

\CuTe 的数学基础与代数化方法，使它能够随着架构创新持续演进。将布局关注点与算法逻辑分离，并提供强大的编译期推理能力，\CuTe 确立了一种面向张量中心编程的方法论，能够满足各类计算系统与应用的需求。

\section*{致谢}
\addcontentsline{toc}{section}{致谢}

作者感谢 Vijay Thakkar 作为 \CuTe 的早期使用者，感谢他在 \CuTe 的普及及其与 CUTLASS 的集成中发挥的关键作用，以及他对 \CuTe 基础设计与部署的重要贡献。他还感谢 Andrew Kerr、Pradeep Ramani 与 Manish Gupta，感谢他们对于在 CUTLASS 中使用 \CuTe 的洞见，以及他们推动 \CuTe 在高性能 kernel 开发中的早期采用。他感谢 Muhammad Osama 与 Duane Merrill 在 \CuTe 原型上的早期实验工作，以及在最初 \CuTe 示例中所做的底层性能优化。最后，作者感谢 Michael Garland、Bastian Hagedorn、Jay Shah 与 Amanda Liu 提出的独到反馈，以及他们对 \CuTe 规范一次次提出的挑战与质询。
