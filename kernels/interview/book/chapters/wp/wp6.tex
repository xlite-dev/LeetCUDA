...
布局可以是单射（injective）、满射（surjective）或双射（bijective）的，并且存在右逆（right-inverse）、左逆（left-inverse）、全逆（full-inverse）与拟逆（quasi-inverse）。当把布局解释为从坐标到偏移的函数时，逆布局便可解释为从偏移到坐标的函数。布局的逆在以下方面非常有用：确定某些偏移在布局中的位置、提取由特定偏移构成的组，或确定两个布局各自的公共子布局。
...
\end{align}
当 $\v{L}$ 具有一般的整数半模（integer-semimodule）陪域（codomain）$\D$ 时，上述一般性定义有助于确定 $\v{L}^{\ddagger}$ 的性质。在常见情形 $\D = \Z$ 下，可以恢复出右逆的经典定义，
\begin{align*}
...
右逆在检查数据布局、判断连续元素是否存在及其所在位置时极为有用。举一个直接的例子：布局 $(4,8):(1,4)$ 与 $(4,8):(8,1)$ 的右逆分别是 $32:1$ 与 $(8,4):(4,1)$。这意味着，由于两个布局的右逆尺寸均为 32，它们都索引到 32 个连续的物理元素。

一个更复杂的例子是 \CuTe{} 中一种常见的拷贝模式——向量化拷贝（vectorizing-copy）：它试图找出两个张量之间一次最多能同时拷贝的元素个数。右逆使 \CuTe{} 能够确定两个布局之间的{\em 最大公共子布局}（maximum common sublayout）；再结合有关硬件能力以及指针与步幅物理对齐的附加信息，就能以代数方式确定可安全向量化的元素数量及其位置，从而完成拷贝。
...
在实践中，考虑第~\ref{sec:copy} 节中的参考函数 {\tt COPY}。该实现可以在不改变可观察行为的前提下，在内部置换迭代顺序。置换迭代顺序等价于对源张量与目标张量的布局施加同一个一致的置换。通过选取与式~\eqref{eqn:common_layout} 所定义布局相匹配的置换，所有连续元素都可以被置换到第一个模中，从而由向量化指令做特殊处理。进一步地，这一过程不仅能检测待向量化的元素是否存在及其位置，还能确定可安全向量化的最大元素数。这就为{\bf 自动向量化}（AutoVectorization）优化机会提供了一个框架。
...
\end{align}
左逆可能不唯一，并且对于不在 $\v{L}$ 的像（image）中的输入，左逆可以取任意值。当 $\v{L}$ 具有一般的整数半模陪域 $\D$ 时，上述一般性定义有助于确定 $\v{L}^{\dagger}$ 的性质。在 $\v{L}$ 为单射这一常见情形下，可以恢复出左逆的经典定义，
\begin{align*}
...
左逆有助于确定某个数据布局所产生的特定偏移是否存在及其所在位置。
...
向 Blackwell 的“张量内存”（tensor memory，TMEM）加载以及从其存储数据所使用的指令，会以一组预定义且非常特定的偏移访问 TMEM，如图~\ref{fig:tmem-fragments} 所示。物理 TMEM 的编址是一个二维网格，最多包含 512 个“列”（col）与 128 个“通道”（lane）。图中展示了每条指令单个 warp 的访问模式，但每条指令实际上使用四个 warp，上述模式每 32 个通道重复一次、共重复四次。在对物理 TMEM 编址时，沿 32 位的“列”方向跨越一步使 TMEM 地址增加 1，沿“通道”方向向下跨越一步使 TMEM 地址增加 16384。基于这一物理编址，我们定义记录每条指令所访问的全部物理偏移的布局：
\begin{center}
\begin{tabular}{l | r@{:}l}
