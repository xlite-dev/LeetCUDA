...
\item {\em 一种定义在布局之上的新颖操作代数}：\CuTe 布局支持一组丰富的操作，包括拼接（concatenation）、合并（coalescence）、复合（composition）、补（complementation）、切分（division）、铺瓦（tiling）与求逆（inversion），它们的结果都仍是新的 \CuTe 布局。这些操作支撑现代张量指令所需的复杂分块（partitioning）、操纵、验证与推导。
\end{compactitem}

\CuTe 的布局表示为编写通用算法提供了一个直观的框架，用于管理线程与数据；\CuTe 的布局代数为高性能线性代数 kernel 的开发提供了一种富有表达力的途径，用于操纵布局并生成新布局。这些方法能够带来：
\begin{compactitem}
\item 对复杂布局与分块的支持：\CuTe 便于表示应用特定数据模式所需的精细布局，以及专用张量指令所需的复杂分块模式。
\item 关注点分离：数据布局独立于算法逻辑进行声明，提升清晰性与模块化。
\item 静态分析与优化：精深的代数技术使我们能够依据体系结构约束对张量参数进行检查、重排与分块。
\end{compactitem}
...
类 BLAS 库实例化软件（BLIS）框架（Van Zee 与 van de Geijn，2015）扩展了 GEMM，使其同时支持行模与列模中的非单位步幅，从而在不借助显式内存拷贝的情况下解决了处理不规则内存布局的部分挑战。BLAS 的跨步批量（strided-batched）GEMM 扩展进一步推广了该原语，使其能够应用于更多张量收缩（Shi 等，2016）。在对矩阵布局做更进一步的抽象时，启发 \CuTe 的一个关键洞察是：在张量记号中使用多重索引，使任意张量收缩都能变换为规范化的 batched-GEMM 原语。
...
对稠密张量表示的独立推广还包括 HeLayers（Aharoni 等，2023）、ThunderKittens（Spector 等，2024），以及 OpenAI 的 Triton 编译器（Tillet 等，2019）中所使用的 Linear Layouts（Zhou 等，2025）方法。ThunderKittens 为寄存器内存、共享内存、行主序/列主序瓦片、由行主序/列主序子瓦片组成的行主序/列主序瓦片，以及为 warp 与线程规定的访问模式实现了种类繁多的定制类型。这些类型的编写考虑了体系结构布局需求与内存层次，但其表示范围局限于既有的类型与模式。Linear Layouts 基于 $\mathbb{F}_2$ 线性代数，为张量布局提供了更一般的表示，也为布局分析与生成开辟了途径。但 Linear Layouts 对 $\mathbb{F}_2$ 的严格依赖使其中一些算子难以被人工检视，并将该工作限制在 2 的幂形状与步幅上，这对许多应用来说是不可接受的。

另一些启发了本工作部分内容的布局分析方法同样依赖 $\mathbb{F}_2$ 线性代数，例如 Edelman 等（1994）、Cormen（1993）与 Bouverot-Dupuis 及 Sheeran（2023）的工作。这些工作为分析与算法生成奠定了基础，但受限于其撰写时的应用语境。
...
\CuTe 的设计与概念已在若干应用中证明了其效用。例如，\CuTe 已被用于 Graphene 张量编译器（Hagedorn 等，2023），在其中扮演表示张量运算的关键角色。此外，\CuTe 的 C++ 实现已被用于 Stream-K 算法（Osama 等，2023）的实现，并且是 NVIDIA CUTLASS v3 库（Kerr 等，2017）开发的核心。\CuTe 还一直是大型语言模型最先进实现中的核心组件，包括 FlashAttention 及其不断演进的各代（Dao 等，2022、Dao，2023、Shah 等，2024），这凸显了它与深度学习前沿研究及应用的关联。此外，\CuTe 还是一系列相关编译器项目的基础，包括 CuTe DSL（NVIDIA，2025）——一种面向线性代数应用的、基于 Python 的动态编译 CUDA 软件的 DSL。
...
此外，规范化循环形式还为可证正确的循环变换提供了指导，这类变换经常出现在面向张量计算的优化编译器中。考虑最一般的规范化循环嵌套：
\begin{cpp}
...
随后，优化便可专注于循环重排、铺瓦、向量化以及其它常见优化，其方式是变换循环嵌套的顺序与阶。事实证明，这些变换往往可以用布局表示为算法坐标空间上的置换。这些变换布局与数据布局进行函数式复合，生成新的循环嵌套，并保证与原始问题一致。布局复合及其在通用分块中的应用细节见第~\ref{sec:composition} 节。
