...
\textbf{计算怎么分给线程}。一条 \texttt{mma.sync.aligned.m16n8k16} 是硬件
...
MMA 原子的 fragment 布局是什么（3.2 节）；TiledMMA 如何由「原子 $+$ warp
排布 $+$ tile 尺寸」构造出来（3.3 节）；\texttt{partition\_fragment\_A/B/C}
如何把 Tensor 切成 $(\mathrm{MMA}, \mathrm{MMA}_M, \mathrm{MMA}_K)$ 形状的
片段（3.4--3.5 节）；以及同一份寄存器为什么能在 QK 与 PV 两个不同的
TiledMMA 之间「原地复用」（4.5 节）。
...
ffpa\_attn.cuh~L31--79——\texttt{FFPAAttnSplitDCuTeTraits} 的完整类型推导，
...
\textbf{编译宏}：\texttt{NOTES\_V2\_ENABLE\_CUTE}。配套最小测试
\texttt{book/tests/ch22\_tiled\_mma.cu}：host 端 partition 一致性断言。
预计篇幅约 13 页。

\section{问题与动机}

回忆第~\ref{ch:11}~章手写 HGEMM 的寄存器清单：一条 m16n8k16 要 4 个 A 寄存器
（8 个 half）、2 个 B 寄存器（4 个 half）、4 个 C 寄存器（float）。这些数字
不是自由选择的，而是 PTX 规定的 fragment 布局：以 A（$16\times16$，列主序的
$(m,k)$）为例，lane 与元素的对应是
...
外加「$+8$ 行」与「$+8$ 列」两套偏移把 8 个 half 填满。$16\times8$ 的 C 也是
类似的故事。手写版的做法是：把这些式子连同 ldmatrix 的 lane 地址表一起
写在 kernel 里。

现在把它放大到 tile 级别。FFPA 的 QK 步骤要算 $S_{64\times64} = Q_{64\times64}
K^T_{64\times64}$（每个 d-chunk），PV 步骤要算 $O_{64\times64} = P_{64\times64}
V_{64\times64}$——两个 $64\times64$ 的结果，每个线程要用多条 m16n8k16 拼出来。
「拼」的规则至少包括：
...
CuTe 的答案是把这四件事压进一个类型：\texttt{TiledMMA<MMA\_Atom,
AtomLayoutMNK, PermutationMNK>}。一个 TiledMMA 实例只回答两个问题：
\textbf{「这个 tile 有哪些 (线程, 值) 位置」}与\textbf{「给定一个 Tensor，
每个线程的 fragment 是什么形状」}。本章要验证的，就是这两个回答在 FFPA 的
具体配置下能否用手推公式复现——不能复现就说明没读懂。

\section{数学原理}

\subsection{MMA 原子：五个类型成员}

CuTe 用一个 traits 类描述一条 MMA 指令（\texttt{MMA\_Traits<Op>}），FFPA 用到
的 \texttt{SM80\_16x8x16\_F32F16F16F32\_TN} 实测（CUTLASS v4.6.1）为：
...
（第~\ref{ch:20}~章式~(20-colex-decomp)）线性化。把 stride 反解回坐标，就得到
三条可以独立验证的公式（lane $=\mathrm{thr}$，值编号 $v$ 按 colex 拆位）：
...
\texttt{make\_tiled\_mma(atom, AtomLayoutMNK, PermutationMNK)} 的第二个参数
是\textbf{warp 排布}（哪个 warp 覆盖哪个原子位置），第三个是\textbf{目标 tile
尺寸}（含 K）。内核把两者合成为
...
\begin{equation}
  ...
  \begin{cases}
    ...
    \mathrm{size}_I \cdot \mathrm{size}_{I+1}, & \text{否则},
  \end{cases}
  \label{eq:22-tile-size}
\end{equation}

给出：QK 实测 $\mathrm{tile\_size} = (64,64,16)$，PV 为 $(64,16,16)$。
式~\eqref{eq:22-tile-size} 第二行里第一个因子是
$\mathrm{Shape\_MNK}$ 第 $I$ 个 mode 的长度，第二个因子是
$\mathrm{ThrLayoutVMNK}$ 第 $I+1$ 个 mode 的长度（后者即「属于该方向的
线程组数，再加一」的位置：对 M 是 warp 排布的 $\mathrm{ThrM}$，对 N 是
$\mathrm{ThrN}$，对 K 是 $\mathrm{ThrK}$）。

\subsection{q 的推导链：从 tile 到 fragment}

真正的工作在 \texttt{thrfrg\_A/B/C} 里，它是四步 layout 代数
（第~\ref{ch:20}~章的全部工具在这里合体）。以 A 为例（B/C 只是把 mode 换成
$(N,K)$ / $(M,N)$）：
...
\begin{itemize}
  \item (1)  $\mathrm{perm} = (64,16)$：$T_1 = ((1,64),(4,16))$；
...
\texttt{gemm(tiled\_mma, A, B, C)} 会在这一层 remainder 上循环——这正是
第~\ref{ch:21}~章 kernel 里「只显式展开 K 循环、M/N 交给 \texttt{cute::gemm}」
的原因（hgemm.cuh~L925--937 的注释）。

\subsection{get\_slice：线程坐标的三层解构}

\texttt{thr\_mma = tiled\_mma.get\_slice(t)} 做的事情只有一行：把整数 $t$ 按
式~\eqref{eq:22-thr-vmnk} 的 layout 分解成 $(\mathrm{ThrV},(\mathrm{ThrM},
\mathrm{ThrK}))$（C 用 $(\mathrm{ThrV},(\mathrm{ThrM},\mathrm{ThrN}))$）。
...
\texttt{get\_thread\_slice} 是 \texttt{get\_slice} 的别名（v4.6.1 实测同一
实现），旧教程里两个名字都可能出现。
...
\begin{itemize}
  \item \texttt{SmemLayoutAtom = GMMA::Layout\_K\_SW128\_Atom<half>}：smem 的
        布局原子（第~\ref{ch:23}~章的 SW128），本 traits 只用它定义
        Q/KV 的 smem 视图；$64\times64$ 的 tile 实测
        \texttt{cosize} $=4096$（与 $64\times64$ 个 half 一致，无空洞）；
  \item \texttt{SmemLayoutVt}：把 KV 的布局\emph{转置}成「$d$ 为行、$kv$ 为列」，
        供 PV 的 B（$V^T$）使用——这是 ch19 讲过的「$V$ 以转置形态喂给
        MMA」的布局表达；
  \item \texttt{MmaAtom = MMA\_Atom<SM80\_16x8x16\_F32F16F16F32\_TN>}：
  ...
  \item 两个 \texttt{Copy\_Atom}：\texttt{LDSM\_N}（正常读）与
        ...
        \texttt{make\_tiled\_copy\_B} 的原子参数。
\end{itemize}
...
（第~\ref{ch:19}~章的寄存器压力讨论）。若把 PV 的 N 也拉到 64，单次 MMA 的
C fragment 会从 8 涨到 32，寄存器直接爆掉。实测 PV 在 $(64,16)$ 的 C 上
每线程 $(4,1,2) = 8$ 个 float，与注释「EURepeat$=<1,2,1>$」一致——
\textbf{tile 的 N 尺寸就是 acc\_O 寄存器的旋钮}。
...
注意 \texttt{OFragType} 是用 \texttt{Shape<\_,64,\_64>} 实例化的——虽然 PV
的 tile 是 $64\times16$，这里刻意按 $64\times64$ 取 fragment（$N$ 方向 4 个
tile 重复），因为一个 d-chunk 的 $O$ 就是 $64\times64$。实测
\texttt{kOElemsPerFrag}$=32$、\texttt{kORows}$=2$、\texttt{kOCols}$=16$——
后两者来自 \texttt{convert\_layout\_acc\_rowcol} 把
...
\begin{sloppypar}
\texttt{partition\_fragment\_A(sQ\_stg)} 的输入是 $(64,64)$ 的一个
d-chunk 的 Q，返回 $(8,1,4)$ 的寄存器 fragment——式~\eqref{eq:22-fragment}
的实测形态。注意 \texttt{tCrS} 是在循环外一次性定义的
（\texttt{partition\_fragment\_C(tiled\_mma\_qk,}\allowbreak
\texttt{Shape<\_,64,\_64>\{\})}
...
\begin{sloppypar}
\texttt{convert\_type<half>} 做 fp32$\to$fp16 的类型转换（零拷贝，只改元素
类型解释）；\texttt{convert\_layout\_acc\_Aregs<}\allowbreak
...
flash\_attn.cuh~L2341--2378 明确写了这一点，第~\ref{ch:25}~章会再展开）。
PV 一侧的使用（L339--345）：
\end{sloppypar}
...
\texttt{tCrV} 用 \texttt{tCrV\_layout}（在循环外用 \texttt{SmemLayoutVt}
的\emph{非 swizzle 部分}推导，L141--142）重新解释同一批寄存器——
...
\centering
\begin{verbatim}
  FIG-22-1  TiledMMA partition for FFPA QK: Atom = SM80_16x8x16, 128 thr
            AtomLayoutMNK = (4,1,1)   PermutationMNK = Tile<64,64,16>

    warp w covers rows [16w, 16w+16)          (ThrM = 4, atom_M = 16)
    within a warp, one atom = 16 rows x 8 cols:
        lane = 4*(row%8 ... ) ...            (fig 22-1a: atom fragment)
        m = lane/4 + 8*d1 ;  n = 2*(lane%4) + d0

    +--------------------------------------------------------------+
    |  warp 0: rows  0-15   |  warp 1: rows 16-31                   |
    |  each warp repeats the atom 8x in N:                          |
    |     N atoms:  n = 0..7   ->  cols [8j, 8j+8)                  |
    +--------------------------------------------------------------+
    |  warp 2: rows 32-47   |  warp 3: rows 48-63                   |
    +--------------------------------------------------------------+

    per-thread fragments on the (64,64) tile (measured):
      A (Q): (MMA=8, MMA_M=1, MMA_K=4) =  32 half    (bijection)
      B (K): (MMA=4, MMA_N=8, MMA_K=4) = 128 half    (4x dup: 4 M-warps)
      C (S): (MMA=4, MMA_M=1, MMA_N=8) =  32 float   (bijection)
    conservation: 128 thr x 32 = 4096 = 64 x 64  (A and C)

    PV mma (Tile<64,16,16>) on the same threads:
      C (O): per (64,16) tile -> (4,1,2) = 8 float
             full 64x64 d-chunk view -> (4,1,8) = 32 float per thread
             x8 d-chunks (D=512)      -> 256 registers  <-- the knob
\end{verbatim}
...
N 方向只重复 2 个原子，代价换来 acc\_O 的寄存器可控。本章测试 case~C--F
对全部形状与覆盖做了逐点核对。（占位，RFC-H 升级为 drawio 图）}
\label{fig:22-1}
\end{figure}
...
\textbf{账一（寄存器）}：acc\_O $=kD_{\mathrm{chunks}}\times
\mathrm{MMA}\times\mathrm{MMA}_M\times\mathrm{MMA}_N$。PV 的
$\mathrm{MMA}_N = 2$（$16/8$）时每个 $64\times64$ d-chunk 是 32 个 float；
$D=512$ 时 $8$ 个 chunk 共 256 个寄存器。若把 PV 的 tile N 改成 64
（$\mathrm{MMA}_N = 8$），单 chunk 就是 128 个 float——8 倍超支，
不可能常驻。这就是 FFPA 注释里「PV 保持小 tile 控制 acc\_O 寄存器」的
算术依据。
...
$=\mathrm{MMA}_M\times\mathrm{MMA}_N\times\mathrm{MMA}_K$。QK 的
$(1,8,4) = 32$ 条 m16n8k16 覆盖 $64\times64\times64$——512 KFLOP/32 条，
每条约 16 KFLOP；PV 的 $(1,2,4)=8$ 条覆盖 $64\times16\times64$。
两个 tile 都让「每条 MMA 指令」吃满原子尺寸，没有浪费——因为它们都是从
$\mathrm{AtomShape}$ 出发、只用排布与 tile 尺寸做整数分块推出来的。

\textbf{口径说明}：本节所有数字都是 host 端类型推导的实测值（
\texttt{partition\_fragment\_*} 的形状），不含计时数据；FFPA 的
TFLOPS 对照见第~\ref{ch:19}~章的表与第~\ref{ch:26}~章的 kernel 分析。

\section{勘误与考据}

\begin{kaogu}[F1 核校通过：双 TiledMMA 的注释与实测一致]
(a) L50--53 注释「QK: Tile<64,64,16> $+$ Layout<4,1,1> $\to$ EURepeat<1,8,1>，
一次覆盖 S[64,64]」——实测 $\mathrm{tile\_size}=(64,64,16)$、
$\mathrm{MMA}_N=8$（即 N 方向 8 个原子全在一个 warp 内），fragment
$(4,1,8)$ 覆盖整个 $64\times64$ 的 $S$，一致；
(b) L55--58 注释「PV: Tile<64,16,16> $\to$ EURepeat<1,2,1>」——实测
$\mathrm{tile\_size}=(64,16,16)$、fragment $(4,1,2)$（每 $64\times16$ 片
2 个 N 原子），一致；
(c) L47--48 注释「128 producer + 128 consumer = 256 total threads
（vs 原始 384）」——那是 \emph{TMA+WS 版} kernel 的线程配置；本 traits 服务的
cp.async 版是 128 线程（L83--86 注释），两者不矛盾，但读者容易把
「Layout<4,1,1> 一致」误读成 256 线程。实测 \texttt{size(TiledMmaQK)}
$=128$。
\end{kaogu}

\begin{kaogu}[术语考据：「EURepeat」在两种语境下的两种含义]
本章与 hgemm 的注释都在用「EURepeat」，但指向不同的量：
(a) \emph{warp 排布}语境（hgemm.cuh L1283--1290 的
\texttt{kMmaEURepeatM/N/K}，由 \texttt{make\_tiled\_mma} 的第二个参数给出）；
(b) \emph{每线程原子重复}语境（本章 FFPA 注释的 $\langle1,8,1\rangle$ 与
$\langle1,2,1\rangle$，由 tile 尺寸与 warp 排布共同推出，即
式~\eqref{eq:22-fragment} 的 $(\mathrm{MMA}_M,\mathrm{MMA}_N,\mathrm{MMA}_K)$）。
FFPA 的 warp 排布是 $(4,1,1)$ 而「EURepeat」写 $\langle1,8,1\rangle$，
正是因为它在说 (b)。读旧教程时先确认语境，再代公式。
\end{kaogu}

\begin{kaogu}[F4：文件头性能口径为 PRO 5000]
ffpa\_attn.cuh L21--23 的头注释给出「SM120a RTX PRO 5000：cuDNN SDPA
57.2 TFLOPS、FFPA TMA WS 110.7 TFLOPS（1.94x）」。除法自洽
（$110.7/57.2 = 1.94$），但设备口径是 RTX PRO 5000（本机），与 README 的
5090 口径不同——引用数字时必须带上设备（BOOK\_PLAN §5.3 F4 类的已知样本）。
\end{kaogu}

\section{常见坑}

\textbf{坑一：fragment 必须先清零/初始化。}
\texttt{partition\_fragment\_C} 返回的是「一块未定义的寄存器数组」的视图，
...
表现为「结果偶尔对、偶尔全错」。同理，被 \texttt{convert\_layout\_acc\_Aregs}
...
\emph{类型转换 $+$ 布局重解释}两步（\texttt{convert\_type} $+$
\texttt{convert\_layout\_acc\_Aregs}），且只对 m16n8k16 这一族成立。
...
用它来配 \texttt{dim3 block(size(mma))}。
...
\textbf{坑五：\texttt{partition\_fragment\_A} 的输入决定 fragment 的
MMAK 大小，但不改变 atom 约定。}
QK 的 A 在 $(64,64)$ 上是 $(8,1,4)$，在 $(64,16)$ 上就是 $(8,1,1)$；
$\mathrm{MMA}_K$ 随输入 K 长度变化。写循环时要用
\texttt{size<2>(fragment)} 作为 K 循环上界（第~\ref{ch:21}~章 kernel 的
...
\texttt{convert\_layout\_acc\_rowcol}（L2315--2333 推导）；直接用
\texttt{fragment(idx)} 当矩阵元素是最常见的读错方式。
...
参数，并提供 \texttt{partition\_fragment\_*}。

\textbf{Q2：\texttt{make\_tiled\_mma} 的第二个、第三个参数分别是什么？}\\
A：第二个是 warp 排布（\texttt{AtomLayoutMNK}，决定 \texttt{ThrM/ThrN/ThrK}
与重复因子），第三个是目标 tile 尺寸（\texttt{PermutationMNK}，含 K）。
...
\textbf{Q5：\texttt{get\_slice(t)} 内部发生了什么？}\\
A：把整数 $t$ 按 \texttt{ThrLayoutVMNK} 分解成
...
\textbf{Q7：\texttt{tile\_size<I>(mma)} 有什么用？}\\
A：它是 TiledMMA 对外的尺寸接口，也是 \texttt{make\_tiled\_copy\_A/B/C}
推导 S$\to$R 拷贝 tiler 的来源（第~\ref{ch:21}~章式~(21-mtca)）——
TiledMMA 与 TiledCopy 通过它解耦：改 tile 不影响拷贝代码。

\textbf{Q8：为什么说「tile 的 N 尺寸是寄存器旋钮」？}\\
A：C fragment 每线程元素数 $=\mathrm{MMA}\times\mathrm{MMA}_M\times
\mathrm{MMA}_N$，而 $\mathrm{MMA}_N = \mathrm{tile}_N/\mathrm{atom}_N$。
FFPA 在 $D=512$ 时 acc\_O 已达 256 个寄存器，所以 PV 的 $\mathrm{tile}_N$
只能取 16（$64\times16\times16$）。

\section{最小测试与动手实验}

测试文件：\texttt{book/tests/ch22\_tiled\_mma.cu}（host-only：只做类型与
partition 断言，不启动 kernel；直接 include
\texttt{../../ffpa\_attn.cuh} 以核对真实的 traits）。六个 case：

\begin{itemize}
  \item \textbf{A 原子 fragment 公式}：对
        \texttt{SM80\_16x8x16\_F32F16F16F32\_TN} 的 A/B/C 三个 layout，
        逐 lane、逐值核对手推公式式~\eqref{eq:22-atom-a}--\eqref{eq:22-atom-c}
        （$32\times8$、$32\times4$、$32\times4$ 个断言）；
  \item \textbf{B tile\_size 与线程数}：\texttt{FFPAAttnSplitDCuTeTraits<512>}
        的 QK/PV \texttt{tile\_size} 为 $(64,64,16)/(64,16,16)$、
        \texttt{size(mma)}$=128$，并与手写的
        \texttt{make\_tiled\_mma} 构造做类型一致性检查；
  ...
  \item \textbf{F PV 的 acc\_O 账目}：
        \texttt{partition\_fragment\_C(tiled\_mma\_pv, Shape<\_,64,\_64>\{\})}
...
\begin{enumerate}
  \item 把 case~C 的 PV mma 的 tile 从 \texttt{Tile<64,16,16>} 改成
        \texttt{Tile<64,32,16>}，观察 fragment 的 $\mathrm{MMA}_N$ 从 2 变
        4、单 d-chunk 的 acc\_O 从 32 涨到 64 个寄存器——亲手体验「tile N
        是寄存器旋钮」；
  \item 把 QK 的 warp 排布从 \texttt{Layout<Shape<\_,4,\_,\_>>} 改成
        \texttt{Layout<Shape<\_,2,\_2,\_>>}（$2\times2$ 网格），观察
