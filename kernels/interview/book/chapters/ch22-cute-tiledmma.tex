...
先从一条 mma 指令说起。\texttt{mma.sync.aligned.m16n8k16} 不是一个线程的
...
（\texttt{MMA\_Traits<Op>}），FFPA 用到的
\texttt{SM80\_16x8x16\_F32F16F16F32\_TN} 实测（CUTLASS v4.6.1）为：
...
\textbf{单 warp 演练：\texttt{partition\_fragment\_A/B/C} 给出什么？}
...
\texttt{make\_tiled\_mma(atom, AtomLayoutMNK, PermutationMNK)} 的三个参数：
...
给时会怎么算」：原子长度乘该方向的线程组数（M 对应
$\mathrm{ThrM}$、N 对应 $\mathrm{ThrN}$、K 对应 $\mathrm{ThrK}$）。

用映射语言说：\texttt{ThrLayoutVMNK} 把\textbf{线程编号}拆成
...
\texttt{PermutationMNK} 这个名字的由来。常规场景把它当 tile 尺寸读即可。
...
\begin{itemize}
  \item \textbf{注释里的线程数要带语境读}。traits 上方的「128 producer
        ...
        是 128 线程（L83--86 注释）。实测 \texttt{size(TiledMmaQK)}
        $= 128$，与 \texttt{Layout<4,1,1>}（$4\times32$）自洽。看到注释
        数字与实测不一致时，先问「它描述的是哪个版本」，再下结论。
  \item \textbf{「EURepeat」有两种语境}。hgemm.cuh~L1283--1290 的
        \texttt{kMmaEURepeatM/N/K} 说的是 \emph{warp 排布}
        （\texttt{make\_tiled\_mma} 的第二个参数）；本章 FFPA 注释里的
...
对全部形状与覆盖做了逐点核对。图规格：三栏纵排——上栏「线程划分」，四个
等宽横向色带（warp 0--3，各标行区间 $[16w,16w+16)$），带内画 8 个等宽
小方块表示 N 方向 8 个原子；中栏「fragment 账目」，A/B/C 三张卡片各标
$(\mathrm{MMA},\mathrm{MMA}_M,\mathrm{MMA}_K)$ 三元组、元素数与重复因子
（B 卡片加 4 倍重复角标）；下栏「PV 寄存器账」，横向堆叠条形图表示
8 个 d-chunk $\times$ 32 float $=$ 256 寄存器，右端画一条标注「N tile
拉到 64 则 8$\times$ 超支」的警戒线。配色：A 蓝、B 绿、C 橙，warp 色带
灰底黑描边。
...
还有一条引用纪律：ffpa\_attn.cuh L21--23 的头注释给出「SM120a
