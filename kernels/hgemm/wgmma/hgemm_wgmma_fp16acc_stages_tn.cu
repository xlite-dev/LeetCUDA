...
#define IDX(i, j) (((i) + yo) * N + (j))
        ...
        *reinterpret_cast<uint32_t *>(&block_C[IDX(row + 8, col)]) =
            d[m_it][g][1];
        *reinterpret_cast<uint32_t *>(&block_C[IDX(row, col + 8)]) =
            d[m_it][g][2];
...
#define LAUNCH_WGMMA_STAGE_SWIZZLE_TN_KERNEL(stages, stride)                   \
  {                                                                            \
    ...
    cudaFuncSetAttribute(                                                      \
        hgemm_wgmma_m64n128k16_f16acc_stages_tma_ws_tn_kernel<                 \
            WGMMA_M, WGMMA_N, WGMMA_K, BM, BN, BK, NUM_THREADS, (stages),      \
            true>,                                                             \
...
int main(int argc, char *argv[]) {
  ...
  for (int j = 0; j < test_num; j++) {
    ...
    for (int k = 0; k < outer_repeat; k++) {
      double this_sec =
          perf_gemm<half>(launch_hgemm_wgmma_m64n128k16_f16acc_tn<3, 2048>, M,
