...
__global__ void mat_transpose_f32x4_row2col_kernel(float *x, float *y, const int row, const int col)
{
  ...
  if (global_row < row && global_col < col)
  {
    ...
    // y[global_col * row + global_row] = x_val.x;
    // y[global_col * row + global_row + 1] = x_val.y;
    // y[global_col * row + global_row + 2] = x_val.z;
    // y[global_col * row + global_row + 3] = x_val.w;
  }
}
// work for row == col
...
__global__ void mat_transpose_f32x4_col2row2d_kernel(float *x, float *y, const int row, const int col)
{
  ...
  if (global_x * 4 + 3 < col && global_y < row)
  {
    ...
    // float4 x_val;
    // x_val.x = x[global_y * col + global_x * 4];
    // x_val.y = x[global_y * col + global_x * 4 + 1];
    // x_val.z = x[global_y * col + global_x * 4 + 2];
    // x_val.w = x[global_y * col + global_x * 4 + 3];
...
__global__ void mat_transpose_f32x4_shared_col2row2d_kernel(float *x, float *y, const int row, const int col)
{
  ...
  // __shared__ float tile[WARP_SIZE_S][WARP_SIZE_S];
  // if(global_x * 4 < col && global_y < row){
  //   reinterpret_cast<float4*>(tile)[local_y *local_x] = reinterpret_cast<float4*>(x)[global_y * col / 4 + global_x];
  //   __syncthreads();
  //   float4 smem_val;

  //   smem_val.x = tile[local_x][local_y];
  //   smem_val.y = tile[local_x + 1][local_y];
  //   smem_val.z = tile[local_x + 2][local_y];
  //   smem_val.w = tile[local_x + 3][local_y];
  //   const int bid_x = blockIdx.y * blockDim.y;
  //   const int bid_y = blockIdx.x * blockDim.x;
  //   FLOAT4(y[(bid_y * 4 + local_x * 4 + local_y % 4) * row + (bid_x / 4 + 4 * (int)(local_y / 4))]) = FLOAT4(smem_val);
  // }
}
...
__global__ void mat_transpose_f32x4_shared_bcf_col2row2d_kernel(float *x, float *y, const int row, const int col)
{
  ...
  // const int local_x = threadIdx.x;
  // const int local_y = threadIdx.y;
  // __shared__ float tile[WARP_SIZE_S][WARP_SIZE_S + 1];
  // if(global_x * 4 < col && global_y < row){
  //   tile[local_y][local_x * 4] = x[global_y * col + global_x * 4];
  //   tile[local_y][local_x * 4 + 1] = x[global_y * col + global_x * 4 + 1];
  //   tile[local_y][local_x * 4 + 2] = x[global_y * col + global_x * 4 + 2];
  //   tile[local_y][local_x * 4 + 3] = x[global_y * col + global_x * 4 + 3];
  //   // FLOAT4(tile[local_y][local_x * 4]) = FLOAT4(x[global_y * col + global_x * 4]);
  //   __syncthreads();
  //   float4 smem_val;
  ...
  //   const int bid_x = blockIdx.y * blockDim.y;
  //   const int bid_y = blockIdx.x * blockDim.x;
  //   // this index transformation is so hard for me.
  //   constexpr int STRIDE = WARP_SIZE_S / 4;
  //   FLOAT4(y[(bid_y + local_x * 4 + local_y % STRIDE) * row + (bid_x / 4 + 4 * (int)(local_y / 4))]) = FLOAT4(smem_val);
  // }
}
...
#define TORCH_BINDING_MAT_TRANSPOSE2D(tag, th_type, element_type, n_element_row, n_element_col)                              \
  ...
  {                                                                                                                          \
    ...
    dim3 grid((N + WARP_SIZE_S - 1) / (WARP_SIZE_S * n_element_col), (M + WARP_SIZE_S - 1) / (WARP_SIZE_S / n_element_row)); \
    mat_transpose_##tag##2d_kernel <<<grid, block >>> (reinterpret_cast<element_type *>(x.data_ptr()),                       \
                                                          reinterpret_cast<element_type *>(y.data_ptr()), M, N);             \
  }
// 1d index
...
// 2d index. easier for diagonal and shared memory optimize
