...
lib = load(name='hgemm_lib', sources=get_build_sources(), 
...
for (M, N, K) in zip(Ms, Ns, Ks):
    ...
    PERF_COUNT += 1
    pretty_print_line()
    pretty_print_line(f"M={M}, N={N}, K={K}, Warmup={args.warmup}, "
                      f"Iters={args.iters}, {PERF_COUNT}/{len(Ms)}", 
                      sep=" ")
    pretty_print_line()
    a = A[:M, :K].contiguous()
    b = B[:K, :N].contiguous()
