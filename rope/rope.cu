...
__global__ void _rope_f32_kernel(float* x, float* out, int token_len, int N){ 
...
__global__ void rope_f32x4_pack_kernel(float* x, float* out, int token_len, int N){ 
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
void _rope_f32(torch::Tensor x, torch::Tensor out){
    int seq_len = x.size(0);
    int hidden_size = x.size(1);
    int N = (int)(hidden_size/2);
    dim3 grid(seq_len);
    dim3 block(N);
    _rope_f32_kernel<<<grid, block>>>(x.data_ptr<float>(), out.data_ptr<float>(), seq_len, N);
}
...
void rope_f32(torch::Tensor x, torch::Tensor out){
    ...
    rope_f32_kernel<<<grid, block>>>(x.data_ptr<float>(), out.data_ptr<float>(), seq_len, N);
}
...
void rope_f32x4_pack(torch::Tensor x, torch::Tensor out){
    ...
    dim3 grid((seq_len * N + BLOCK_SIZE - 1) / BLOCK_SIZE);
    dim3 block(BLOCK_SIZE);
    rope_f32x4_pack_kernel<<<grid, block>>>(x.data_ptr<float>(), out.data_ptr<float>(), seq_len, N);
}
